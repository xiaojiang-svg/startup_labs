...
static void PeriphCommonClock_Config(void); // 外设时钟初始化

/* 全局变量 */
int mainLoopFlg;
// uint32_t sd_buffer[SD_BLOCK_SIZE / 4];
/* OS */
...
int main(void) {
  ...
  PeriphCommonClock_Config(); // 配置外设时钟SDMMC为100MHz
 ...
 * @param  None
 * @retval None
 * @note 暂时未对PLL2和PLL3进行配置，如果后面有需要用到PLL2和PLL3再进行补充
 */
static void SystemClock_Config(void) {
  ...
  RCC_OscInitStruct.PLL.PLLQ = 24; // SPI1/2/3时钟源均为40MHz，方便在识别卡阶段配置分频；SDMMC1时钟源为40MHz
